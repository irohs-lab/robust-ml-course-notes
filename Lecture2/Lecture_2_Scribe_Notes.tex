...
\subsubsection{ Convexity}\label{sec:Convexity}
    ...
    \item $l(h,z)$ \text{must be a convex function in } $h$ \\ \Rightarrow $\forall z, l(\alpha h+(1-\alpha)h',z) \leq \alpha l(h,z) + (1-\alpha)l(h',z)$
...
\Rightarrow \lvert\lvert\nabla l(z,h)\rvert\rvert \leq \rho$ where ${z,h}$ and ${z, h'}$ \in \text{the domain of function $l$, i.e., the gradient of function can't be infinite.} 
\subsubsection{Smoothness}\label{sec:Smoothness}
We say a function $l$ to be smooth if,
$\forall z,h,h' \lvert\lvert \nabla l(z,h) - \nabla l(z,h')\rvert\rvert \leq \beta\lvert\lvert h-h'\rvert\rvert$ where ${z,h}$ and ${z, h'}$ \in \text{the domain of function }$l$, and $\beta$ is some constant.
